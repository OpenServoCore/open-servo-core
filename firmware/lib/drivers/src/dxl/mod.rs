...
//! ([`DxlBus`] doc). `firmware/ch32/src/runtime/registry.rs` binds each
//! to its V006 value. Future sub-drivers (`DxlTx`, `DxlChainCatchup`)
//! land as additional fields.
...
/// gets:
///
/// - `U`: USART baud-rate setter (sub-driver `DxlClock`).
/// - `T`: HSI / HSE trim setter (sub-driver `DxlClock`).
/// - `R`: DMA-ring ISR surface for DMA1_CH7 (sub-driver `DxlRx`).
...
///   Also drives the BT ring depth inside `DxlRx` — doc §8.3 requires
...
> {
    rx: DxlRx<R, EDGE_BUF_LEN, RX_BUF_LEN>,
    clock: DxlClock<U, T>,
...
impl<
    ...
    const RX_BUF_LEN: usize,
    const EDGE_BUF_LEN: usize,
> DxlBus<U, T, R, CRC, DECODER_CAP, RX_BUF_LEN, EDGE_BUF_LEN>
{
    pub fn new(
        rx: DxlRx<R, EDGE_BUF_LEN, RX_BUF_LEN>,
        clock: DxlClock<U, T>,
...
mod tests {
    ...
    type TestRx = DxlRx<FakeDmaRing, EDGE_BUF_LEN, RX_BUF_LEN>;
    type TestBus = DxlBus<
        FakeUsartBaud,
        FakeClockTrim,
        FakeDmaRing,
    ...
    fn make_bus_with(rx: TestRx, baud: BaudRate) -> TestBus {
        DxlBus::new(rx, make_clock(baud), TEST_ID, TEST_RDT_US)
    }

    fn make_bus() -> TestBus {
        make_bus_with(DxlRx::new(FakeDmaRing::default()), BaudRate::B3000000)
    }
