...
pub(crate) struct ReplyContext {
    ...
    /// `Some(Last { .. })` starts the Fast Last CRC fold pipeline so our
    ...
    /// k > 0 — the sequence-driven start path of `docs/dxl-streaming-rx.md`
    ...
    /// Which ISR triggered the parser's Crc event (HT/TC vs USART IDLE). The
    /// Fast slot send path floors the effective RDT by the source's
    /// `now − packet_end` offset so slot 0's wire-start wall doesn't slip behind
