...
mod tests {
    ...
    fn on_fold_step_drains_raw_bytes_into_fold() {
        ...
        // fold_start_cursor = req.len(), starts the fold there, and its
        // inline body folds that byte. Stage three more predecessor bytes
        // past it — fewer than the 12-byte bytes_before, so the fold stays
        // short of target — and expect the next body to drain all three.
        ...
        assert_eq!(after - before, more.len() as u32);
    }
    ...
    fn on_time_observation_past_inline_horizon_leaves_fold_to_the_grid() {
        ...
        assert!(bus.fast_last.grid_active());
        assert_eq!(
            bus.fast_last.bytes_folded(),
            0,
            "no inline body — nothing folded at observation time",
        );
    }
