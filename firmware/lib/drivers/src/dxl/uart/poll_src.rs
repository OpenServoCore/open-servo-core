...
//! codec (packet-end formulas) and the clock (slot-deadline floor) branch
//! on it, so it lives above both sub-drivers rather than inside either one.
...
/// `now` into a packet-relative tick: the codec's packet-end estimate and
/// the clock's per-source slot-RDT floor. The two contexts have different
