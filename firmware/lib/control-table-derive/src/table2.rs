...
pub fn expand(input: &DeriveInput) -> syn::Result<TokenStream2> {
    ...
    let Data::Struct(s) = &input.data else {
        return Err(syn::Error::new(
            input.span(),
            "FlatTable can only be derived for structs",
        ));
    };
    let Fields::Named(fields) = &s.fields else {
        return Err(syn::Error::new(
            input.span(),
            "FlatTable requires a struct with named fields",
        ));
    };
    ...
    let size = attrs.size.ok_or_else(|| {
        syn::Error::new(
            table_ty.span(),
            "FlatTable requires `#[ct_table(size = ...)]`",
        )
    })?;
    ...
    Ok(quote! {
        ...
        const _: () = {
            assert!(
                ...
                "FlatTable struct size does not equal its declared size; add an explicit `_rsvd` tail array to fill the table budget",
            );
            assert!(
                0 #(+ #size_terms)* == ::core::mem::size_of::<#table_ty>(),
                "FlatTable struct has repr(C) padding; add explicit `_rsvd` fields so the flat read path never exposes uninitialized bytes",
            );
    ...
    for a in attrs {
        ...
        a.parse_nested_meta(|m| {
            if m.path.is_ident("C") {
                ...
                Err(m.error("FlatTable derive forbids #[repr(packed)] (unaligned reads + offset_of are unsound)"))
                ...
                Err(m.error("FlatTable derive requires #[repr(C)] and forbids alternate reprs"))
            }
        })?;
    }
    if !has_c {
        return Err(syn::Error::new(
            struct_span,
            "FlatTable derive requires #[repr(C)]",
        ));
    }
    Ok(())
}
