...
use osc_integration::sim::{Host, Servo, Sim, format_hex, parse_status, parse_status_stream};
use rstest::rstest;
...
fn reg_write_alone_replies_ok_and_does_not_mutate_table(baud_idx: u8, rdt_us: u32) {
    ...
    sim.with_host(host, |h| {
        h.send_reg_write(Id::new(1), lifecycle::TORQUE_ENABLE, TORQUE_ON);
        h.wait_for_reply();
    });
...
fn chain_of_two_reg_writes_commits_atomically_on_action(baud_idx: u8, rdt_us: u32) {
    ...
    sim.with_host(host, |h| {
        h.send_reg_write(Id::new(1), lifecycle::MODE, MODE_PID);
        h.wait_for_reply();
    });
    sim.with_host(host, |h| {
        h.send_action(Id::new(1));
...
fn reg_write_to_ro_field_replies_access_error(baud_idx: u8, rdt_us: u32) {
    ...
    sim.with_host(host, |h| {
        h.send_reg_write(Id::new(1), identity::FIRMWARE_VERSION, &[0x42]);
        h.wait_for_reply();
    });
...
fn reg_write_broadcast_stages_silently_action_broadcast_commits_silently(
    ...
    sim.with_host(host, |h| {
        h.send_reg_write(Id::BROADCAST, lifecycle::TORQUE_ENABLE, TORQUE_ON);
        h.wait_for_reply();
    });
    sim.with_host(host, |h| {
        h.send_action(Id::BROADCAST);
        h.wait_for_reply();
    });
...
fn reg_write_silent_when_srl_is_read_but_action_still_commits(baud_idx: u8, rdt_us: u32) {
    ...
    let rx_after_reg = sim.host(host).rx_bytes();
    assert!(
    ...
    let rx_after_action = sim.host(host).rx_bytes();
    assert!(
...
fn reg_write_under_torque_lock_replies_access_error(baud_idx: u8, rdt_us: u32) {
    ...
    sim.servo(servos[0]).set_torque_enabled(true);

    sim.with_host(host, |h| {
        h.send_reg_write(Id::new(1), comms::ID, &[NEW_ID]);
        h.wait_for_reply();
    });
    sim.with_host(host, |h| {
        h.send_action(Id::new(1));
...
fn inline_write_between_reg_writes_does_not_drain_staged_queue(baud_idx: u8, rdt_us: u32) {
    ...
    sim.with_host(host, |h| {
        h.send_write(Id::new(1), lifecycle::GOAL_VELOCITY, &velocity_payload);
        h.wait_for_reply();
    });
    sim.with_host(host, |h| {
        h.send_action(Id::new(1));
