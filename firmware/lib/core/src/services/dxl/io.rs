...
pub trait DxlIo {
    ...
    /// Fast Sync/Bulk Read Last/Only path: snoop incoming CRC from `frame_end..`,
    /// fire TX at `idle_tick + fire_us`, patch the 2-byte placeholder before DMA
    /// pre-fetches it. `tx_buf` must already hold payload + placeholder from
    /// `FastSlot::{Last, Only}`. `switch_us == fire_us` signals `Only` (no slaves
    /// to snoop).
