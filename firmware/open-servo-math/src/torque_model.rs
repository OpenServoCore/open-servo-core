//! Torque limiting model for motor current protection.
//!
...
//! gears) by limiting torque through current control.
//!
//! This module contains only the mathematical state machine. Hardware
//! integration and safety logic are handled by the TorqueLimiter in
//! the safety module.

use crate::{Duty, MilliAmp};

/// State of the torque limiting system.
...
/// Configuration for torque limiting behavior.
...
impl Default for TorqueConfig {
    fn default() -> Self {
        Self {
            limit_ma: 500,              // 500mA limit for SG90 protection
            hysteresis_ma: 50,          // 50mA hysteresis band
            deglitch_samples: 3,        // 3 consecutive samples
            backoff_factor_q8: 225,     // 0.88 backoff factor
            recovery_rate: 3277,        // 10% per second recovery
        }
    }
}

/// Pure mathematical torque limiting model.
///
...
impl TorqueModel {
    /// Create a new torque model with the given configuration.
...
mod tests {
    ...
    fn test_sustained_overcurrent_triggers_limiting() {
        let mut model = TorqueModel::default();
    ...
    fn test_minimum_duty_cap() {
        ...
        let mut model = TorqueModel::new(config);
