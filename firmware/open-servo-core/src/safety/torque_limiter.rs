//! Torque limiter for motor current protection.
//!
//! Safety wrapper around TorqueModel that integrates with the servo
//! control system. Handles direction change blanking and provides
//! telemetry information.

use open_servo_math::{MilliAmp, TorqueModel, TorqueConfig, LimitState};
...
/// Torque limiter with safety integration.
///
/// Wraps the pure mathematical TorqueModel with additional
/// safety features like direction change blanking.
#[derive(Debug, Clone)]
#[cfg_attr(feature = "defmt", derive(defmt::Format))]
pub struct TorqueLimiter {
    /// Mathematical torque model
    model: TorqueModel,
    ...
    /// Flag indicating if torque is currently limited
    torque_limited: bool,
}
...
impl TorqueLimiter {
    /// Create a new torque limiter with the given configuration.
    pub fn new(config: TorqueConfig) -> Self {
        Self {
            model: TorqueModel::new(config),
            blanking_cnt: 0,
    ...
    pub fn update(&mut self, current: Option<MilliAmp>, pwm_direction: i16, dt_us: u32) {
        ...
        self.torque_limited = self.model.is_limited();
    }
    ...
    /// Check if torque is currently being limited.
    ...
    pub fn set_config(&mut self, config: TorqueConfig) {
...
mod tests {
    ...
    fn test_direction_change_blanking() {
        let mut limiter = TorqueLimiter::default();
