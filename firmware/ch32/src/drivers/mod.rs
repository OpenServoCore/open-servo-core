...
/// Install every static driver instance in this crate from the board wiring
/// and config defaults. Single entry point so bringup doesn't need to know
/// each driver's per-instance install API.
///
/// SAFETY: must be called once during bringup, before any IRQ is unmasked.
pub unsafe fn install_drivers(w: &BoardWiring, defaults: &ConfigDefaults) {
    ...
    unsafe { dxl::clock::DxlClock::install(defaults.dxl_baud) };
}
