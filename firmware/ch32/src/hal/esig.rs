...
/// The factory UID in the §9.2 wire shape: the 96-bit ESIG as 12 little-endian
/// bytes (UNIID1's low byte first, then UNIID2, UNIID3 — RM §19.2),
/// zero-padded to the fixed 16-byte field.
pub fn uid() -> [u8; 16] {
    let mut out = [0u8; 16];
    for w in 0..3 {
        // SAFETY: ESIG is an always-readable factory ROM region (RM §19.1).
