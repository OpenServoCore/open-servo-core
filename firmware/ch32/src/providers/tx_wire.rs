...
impl bus::TxWire for TxWire {
    ...
    fn release(&mut self) {
        ...
        // the shifter is empty and our push-pull drive still holds the
        // line high, so no byte can be mid-reception — the one provably
        // safe instant for the clear's DATAR read (a DATAR read during
        // reception kills the in-flight byte, bench 2026-07-09). A flag
        // whose byte drained before any STATR read never completed the
        // SR-then-DR pair, and with no further RX drains coming it storms
        // the vector once the wire idles (bench signature: the first
        // ack-bearing exchange after a hot-loop leg dies). The clear's
        // trailing STATR read re-arms the drain-self-clear pairing, so
        // running it unconditionally costs the next break nothing (see
        // `clear_rx_errors`).
