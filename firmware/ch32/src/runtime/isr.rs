...
pub fn on_usart1() {
    ...
    if any_err || !tc {
        ...
        // SAFETY: see fn doc — SESSION is installed before this vector
        // unmasks. A2: the break handler resolves complete frames from ring
        // data in place, so it dispatches like the deadline body.
...
/// SysTick compare — one or more framer/chain/rescue deadlines are due
/// (or a `pend_systick` late-arm wake). CNTIF is cleared first: a final
/// deadline body returns without re-arming and a stale-but-latched flag
...
/// into the composite (or the session) is possible.
