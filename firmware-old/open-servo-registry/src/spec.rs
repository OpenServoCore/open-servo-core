...
pub enum Access {
    ...
    /// Write-only.
    WO,
    /// Read-write (RAM, always writable when torque enabled check passes).
    RW,
...
pub struct RegSpec {
    ...
    /// Access control.
    pub access: Access,
    /// Whether this field is persisted to EEPROM.
    ///
    /// EEPROM fields require Torque Enable = 0 for writes.
    /// Note: Some EEPROM fields are RO (e.g., model_number).
    pub eeprom: bool,
    /// Stable identity hash for persistence (FNV-1a of name).
    pub name_hash: u32,
}
...
impl RegSpec {
    ...
    pub const fn new(
        ...
        Self {
            ...
            access,
            eeprom,
            name_hash: fnv1a_hash_str_32(name),
        }
    }
    ...
    /// let spec = RegSpec::typed::<CentiDeg32>("goal_pos_cdeg", 512, Access::RW, false);
    /// assert_eq!(spec.encoding, Encoding::I32Le);
    /// ```
    pub const fn typed<T: UnitEncoding>(
        name: &'static str,
        address: u16,
        access: Access,
        eeprom: bool,
    ...
    ///
    /// Note: EEPROM fields are writable only when torque is disabled;
    /// this method only checks the access type, not torque state.
    pub const fn is_writable(&self) -> bool {
        matches!(self.access, Access::RW | Access::WO)
    }

    /// Check if this register is readable.
    pub const fn is_readable(&self) -> bool {
        matches!(self.access, Access::RO | Access::RW)
    }
    ...
    /// Check if this field is persisted to EEPROM.
