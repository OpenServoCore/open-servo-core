...
use open_servo_hw::config::{
    BoardKinematicsConfig, BoardPolicyConfig, BoardSafetyConfig, BoardThermalConfig, DxlIdentity,
    EepromDefaults,
};
...
// DXL identity constants
impl BoardConfigProvider {
    /// Model number: 0x8001 (open-servo SG90 variant)
    /// Uses 0x8000+ range to avoid conflicts with Robotis servos.
    pub const MODEL_NUMBER: u16 = 0x8001;

    /// Model information: (1 << 24) | (0 << 16) | 1 = v1.0.1
    pub const MODEL_INFORMATION: u32 = (1 << 24) | (0 << 16) | 1;

    /// Firmware version: 1
    pub const FIRMWARE_VERSION: u8 = 1;

    /// Protocol type: 2 (Dynamixel Protocol 2.0)
    pub const PROTOCOL_TYPE: u8 = 2;

    /// Default servo ID: 1
    pub const DEFAULT_ID: u8 = 1;

    /// Default secondary ID: 255 (disabled)
    pub const DEFAULT_SECONDARY_ID: u8 = 255;

    /// Default baud rate: 1 (57600 bps)
    pub const DEFAULT_BAUD_RATE: u8 = 1;
}
...
impl BoardConfigProvider {
    pub fn dxl_identity() -> DxlIdentity {
        DxlIdentity {
    ...
    pub fn eeprom_defaults() -> EepromDefaults {
        EepromDefaults {
            id: Self::DEFAULT_ID,
            baud_rate: Self::DEFAULT_BAUD_RATE,
            secondary_id: Self::DEFAULT_SECONDARY_ID,
        }
    }
