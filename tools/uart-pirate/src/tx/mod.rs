...
//!
//! Why USART3 and not USART2 on PA2/PA3: PA3 has R10=10 kohm to 3V3 on the
...
/// 9600 baud (~1.1 s for 1024 bytes -- we cap shorter on purpose: a stuck
/// TC means something genuinely broke and the host should see a `busy`
/// error rather than the chip pretending to work).
...
/// PB10/PB11 (USART3_TX/RX): PB10 idles as AF open-drain (released, wire
/// held high by the bus pull-ups) and flips to AF push-pull only while a
/// send shifts out -- both edges actively driven at 3M, then the wire is
/// handed back; PB11 is input pull-up. A permanently push-pull idle-high
/// PB10 would clamp any direct-wire (buffer-less HDSEL) device trying to
/// talk: its GPIO can't win the fight the 74LVC2G241 used to win.
///
/// Scheduled sends arm push-pull at schedule time (the kickoff itself is
/// TIM4-hardware-timed, no CPU in the path), so the wire is driven
/// idle-high from schedule to wire-start -- but the host owns the bus
/// when it schedules, so nothing else should be talking then.
fn init_pins() {
...
/// from the ring (measured: the GREAD id byte, ~1/1000 sends at 0.5M,
/// NoEcho). Write-1s touch nothing; only TC clears, race-free.
fn clear_tc_only() {
...
/// receiver reads those as host byte cadence; measured: thread-fed TX
/// polluted the servos' clock-trim windows +30k ppm under hot traffic,
/// 15x the genuine offset and the wrong sign). The TC clear
...
fn init_usart3() {
    USART3.ctlr2().modify(|w| {
        ...
        // detector runs independent of LINEN (fe_probe: 50/50 per shape
        // incl. a 9.9-bit fast-transmitter break), while LINEN=1 makes
        // reception skew-fragile -- a break ending before the detector's
        // own bit-10 sample corrupts the next start-bit adoption (+7k
        // ppm TX skew = up to 39/40 reply corruption with LINEN, 0/40
        // without). Normal-mode reception also rings every break
        // character deterministically, so boundary records are always
        // attached. FE is not the wake: the flag latches fine in
        // isolation (fe_probe), but the app's own STATR reads + RX-DMA
        // DATAR access retire it within nanoseconds -- a level pend that
        // evaporates before the hart takes it.
        w.set_linen(false);
        w.set_lbdl(false);
        w.set_lbdie(true);
    });
...
/// it. Clearing inside `schedule_send_at` would silently kill that
/// injection before the request ever shipped.
...
/// the actual wire-start tick. Wire-start lands ~3.5 us after the call.
///
...
/// Cap on `BRKSEND gap=` (100 us) -- bounded far under the TC-wait
...
/// shape (break = exactly one character time).
///
...
/// `breaks x gap_us`, so both stay well under the host's read timeout.
...
const CAL_LEAD_TICKS: u32 = 1_440; // 10 us at 144 MHz
...
/// osc-native MGMT CAL train (osc-native-protocol.md sec 9.3), spacing kept
...
/// kick + DMA pipeline, ~1 us) so the next mark's TC wait never runs
/// long, while keeping the receiver-visible seam (1 + PAD bit-times --
/// the break's own extra bit plus this pad) inside the servo's
/// chain-pair gate (wire/16, protocol sec 9.3) for every frame down to a bare
/// COMMIT at 3M.
...
/// sub-byte spacing, which the servo's frame-end-work vs next-break
/// timing needs for stress coverage.
///
/// Frame starts ride an absolute tick32 grid (the [`send_cal`] mark
/// pattern): break k feeds at `t0 + sum(frame spans + pad)`, and the whole
/// per-frame handover -- previous TC, M flips, break feed, payload kick --
/// runs inside the mark's critical section. The inter-frame seam is
/// therefore constant by construction, which makes this burst valid FOOD
/// for the servo's differential chain-pair tracker (protocol sec 9.3): a
/// CPU-shaped jittery seam disqualifies every pair.
    ...
    let r = (|| {
        ...
        while i < stream.len() {
            ...
            critical_section::with(|_| -> Result<(), SendError> {
                ...
                // the frame on the wire (measured: 2000/2000 stale hot
                // loops, every interior frame CRC-dead). Sub-us for hot
