...
//!       lands a few us after the command).
...
//!       extra mark (tick32 ticks, <= 100 us) between the break's stop
//!       bit and byte 0 -- the blind-window sweep's knob.
//!   `PULSE`
//!       Fire the ~20 us host marker on the PB0 dbg pin (`dbg.rs`) --
...
//!       the crystal -- the osc-native MGMT CAL train (protocol sec 9.3).
//!   `LOWPULSE us=<n>`
//!       Drive TX low as a GPIO for `n` us (<= 100 ms), then restore AF.
