...
//! boundary recorder's capture), then TC release, RX-error boundary,
//! IDLE bookkeeping. The STATR read at the head arms the SR half of the
//! flags' SR→DR clear pair; the ring DMA's own DATAR access completes
//! it — this path never reads DATAR (a CPU DATAR read kills a
...
fn USART3() {
    ...
    // RX-error service: FE (a break — the boundary this firmware
    // exists to time) or NE/ORE garble, all latched + level-pended.
    // The body records a boundary or bounds the re-fire storm.
    if (statr.fe() || statr.ne() || statr.ore()) && USART3.ctlr3().read().eie() {
        boundary::on_rx_error(now);
    }
    if statr.idle() && USART3.ctlr1().read().idleie() {
        ...
        let rx = critical_section::with(|_| rings::refresh_rx_total());
        ...
        if rx == LAST_IDLE_RX_TOTAL.load(Ordering::Relaxed) {
            ...
            // Bytes flowed — latched RX-error flags have retired via the
            // drain pair, so the boundary event is safe to re-arm.
            boundary::on_rx_progress();
