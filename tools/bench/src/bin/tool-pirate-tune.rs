...
fn stage_classifier(bus: &mut Client, bauds: &[u32], tol: Option<u32>) -> Result<()> {
    println!();
...
fn run_classifier_probe(bus: &mut Client, baud: u32, tol_arg: Option<u32>) -> Result<String> {
    bus.reset()?;
    std::thread::sleep(Duration::from_millis(20));
    while !bus.bbatch(255)?.is_empty() {}

    bus.send_now(&LOOPBACK_PAYLOAD)?;
...
fn stage_stress(bus: &mut Client, seed: u64) -> Result<()> {
    println!();
...
fn run_stress_burst(bus: &mut Client, burst: &[u8], expected_edges: u32) -> Result<()> {
...
fn stage_tx_comp(bus: &mut Client, bauds: &[u32], shots: u32, write: bool) -> Result<()> {
    println!();
    println!("stage 3: tx-comp (residual = stamp[0].tick − LAST?, single 0x55 byte)");
    let current = bus.comp()?;
    println!(
    ...
    if recommended == current {
        ...
        bus.set_comp(recommended.pipe, recommended.bit_q4)?;
        let verify = bus.comp()?;
...
fn collect_tx_comp_residuals(bus: &mut Client, shots: u32) -> Result<Vec<i64>> {
    ...
    for shot in 0..shots {
        ...
        let commanded = bus.last_send_tick()?;
...
fn drain_stamps_until(bus: &mut Client, expected: usize, timeout: Duration) -> Result<Vec<BStamp>> {
    let mut stamps = Vec::new();
    let deadline = Instant::now() + timeout;
    while stamps.len() < expected && Instant::now() < deadline {
        let batch = bus.bbatch(255)?;
        if batch.is_empty() {
