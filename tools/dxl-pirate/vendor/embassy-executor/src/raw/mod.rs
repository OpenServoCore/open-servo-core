...
// LOCAL FORK: forced to critical-section variants for QingKe V4. Stock cfg
// picks run_queue_atomics on any target_has_atomic="ptr" target, but V4B
// lr.w/sc.w execute as plain lw/sw with sc always succeeding (per V4 manual
// §1.1 + [[reference_qingke_v4_atomics_broken]]). The run-queue's CAS-loop
// then silently clobbers ISR enqueues — observed empirically as a
// pirate_chip_tune wedge when BAUD changes tick the executor's enqueue path
// at the same time a USB IRQ wakes the USB task. CS variants use
// critical_section::with, which on qingke is a single `csrrc gintenr` —
// no LR/SC involved. See [[project_dxl_pirate_mie0_wedge]].
#[cfg_attr(target_has_atomic = "ptr", path = "run_queue_critical_section.rs")]
