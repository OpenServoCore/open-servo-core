...
Bench setup expected (per session notes):
  ...
  - Scope ch1 on the data line, ch2 on rev_b's TX_EN test point
  - Scope trigger: ch2 rising edge, Single

Sequence:
  1. Print "ARM SCOPE NOW" and sleep 5 s. Session-startup pings + retune
     have already finished by the time the test body runs, so any TX_EN
     rising edges before this point are flushed. The user arms the scope
     during the sleep.
  2. Drain pending bytes and injector IDLE stamps so the upcoming fire is
     the next bus event.
  3. ARM the injector to emit slot 0 (header + INJ body) 250 µs after the
     host request ends (one DXL RDT).
  4. Send FastSyncRead [INJ_ID, osc_id] with 4 B payload. rev_b detects
     itself as Last, snoops slot 0, fires slot 1 (own body + frame CRC).
  5. The first TX_EN rising edge after step 2 is rev_b's slot-1 fire —
     this is what the scope captures.
...
def test_fast_sync_dut_last_scope_capture(port, osc_id, injector, baud):
    print("\n" + "=" * 60, flush=True)
    print(f"baud={baud}  INJ_ID={INJ_ID}  DUT_ID={osc_id}  length={LENGTH}", flush=True)
