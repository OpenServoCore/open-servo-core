...
First, back-EMF decoupling. The motor generates $K_e\cdot \omega$ volts on its own, so that gets added to the PI output as feedforward. Without it, the loop has to fight the back-EMF with its integrator, and its effective bandwidth collapses as the motor speeds up. With it, the PI only handles the $R\cdot i + L\cdot di/dt$ part, which does not care about speed.

The speed it feeds forward is the trajectory's $\omega^{\ast}$, never an estimate. Two reasons, and either one on its own would be enough. The velocity the loop closes on is the back-EMF estimate, and the back-EMF estimate is this loop's own output read back through the motor, so decoupling against it is circular. The other estimate on offer, the pot observer's $\hat\omega$, rails at rest, which is where a servo spends most of its life. The profile velocity has neither problem. It is a clean number the trajectory generator already computed, and when the cascade is doing its job the motor is on the profile anyway. Current mode and open-loop mode have no profile, so the term is zero there and the PI carries the back-EMF on its own.

Second, supply-voltage compensation. The loop computes a voltage command, but the output is a PWM duty cycle, and duty times supply voltage is the actual applied voltage. Dividing the command by $\hat V_\text{bus}$ makes the loop gain independent of a sagging battery. (There is no divide in the hot path. The supply estimator computes the reciprocal once per medium tick and the fast loop only ever multiplies by it.)
...
That window has a floor, and it is a stated limit of the design rather than a detail. The sample sits at a fixed point inside the drive window, so a short enough window puts it before anything has settled. Measured against a meter and a resistor grid in place of the motor, the crest current on the development board these numbers came off is honest from about 15% duty on a 2S pack. Below that it walks away fast: the 12% rung reads 11% high and the 10% rung 25% high, with nothing about the load changing. On a USB rail the same ladder never gets there at all, and the floor lands somewhere around 30% with a ten point spread on it.
...
where $\hat v_\text{diff}$ is the voltage across the motor terminals and $\hat i$ the winding current, both averaged over a PWM period (averaging is also what lets me drop the $L\cdot di/dt$ term). No encoder anywhere, and it is good enough to close the velocity loop on. It is what the velocity loop actually closes on, which is the subject of two sections further down.

Both of those averages hide a detail, and getting either one wrong is worth most of ten percent. Under slow decay the bridge shorts the winding to itself for the off part of every period, so there is a differential across the terminals only while the bridge drives. The period average is then the duty fraction times the drive-window differential, bridge drops included, which is the right thing to want: those drops are part of the voltage the winding actually saw. The current side is less forgiving. During that same off part the winding current circulates through the two low-side FETs and not through the shunt, so the trough sample is not a small current, it is no current at all, and averaging crest with trough halves the $\hat R\cdot \hat i$ term. The crest sample on its own is the period average, because with centre-aligned PWM it lands in the middle of the on time and the winding current ramps nearly straight from its valley to its peak.

The firmware runs that arithmetic once per fast tick and boxcar-averages it over 1 ms, carried as two half-sums closed at the medium rate, ten fast ticks each, twenty ticks in all at 20 kHz. There is no divide in any of it: the duty fraction, $1/K_e$ and $1/20$ are all constant reciprocals folded into multiplies. Averaging costs lag and buys quiet, and measured against a commutation-ripple reference the trade is generous. A single raw sample scatters by 2.3%, the 1 ms boxcar by 0.62%, a 4 ms boxcar by 0.37%.

Now the error budget, where an earlier version of this document was wrong. The estimate subtracts $\hat R\cdot \hat i$, and I said earlier that $R$ moves 16% over a 40 °C warmup, and I used to write that at high current and low speed the error term is as big as the back-EMF itself. Measured, it is not. Between 30% and 45% duty on a turning shaft the resistive drop is 8 to 10% of $\hat v_\text{diff}$, so the estimate moves by roughly a tenth of whatever relative error $R$ carries, and a six percent speed error would need $R$ to be wrong by 1.7 ohm, which is over a third of it and further off than any measurement of this motor has ever landed.

Where the term genuinely takes over is holding. Holding means current with no motion, $\hat v_\text{diff}$ and $\hat R\cdot \hat i$ are then nearly equal, and the estimate is the small difference of two large numbers. A motor 40 °C into a warmup, holding at 30% duty, invents about 48 motor rev/s that are not there, which is 73 °/s at the output against the roughly 400 °/s a free shaft does at that duty. So this observer is only honest if something tracks $R$ as the motor heats up, and at rest it is not a velocity sensor at all. Tracking $R$ is the next block's job. Staying out of the rest regime entirely is why the pot observer below keeps every decision shaped around standing still.

There is one more failure mode and it is not a graceful one. Both samples sit at fixed points inside the drive window, so below the sampling floor of the previous section the estimate does not decay into noise, it reads a confident negative speed. A sample that misses a floor voids the half it lands in, and a half that is void kills both 1 ms boxcars it belongs to, so the estimate is either reported or absent. Never quietly wrong.
...
You might have noticed the circularity. $\hat\omega_\text{bemf}$ needs $\hat R$, and $\hat R$ needs $\hat\omega$. The fix is timescale separation. The $\hat R$ update only runs when a gate allows it (high current, plus near-zero or encoder-confirmed speed) and moves over seconds, while $\hat\omega_\text{bemf}$ moves in milliseconds. Each one is effectively constant from the other one's point of view. The speed the gate reads is the pot observer's, not the back-EMF's, which is what actually cuts the loop rather than merely slowing it down.

### The Pot Observer

![pot observer](control/pot-observer.svg)

Position and disturbance come from a small fixed-gain observer over three states, $[\hat\theta_m,\ \hat\omega_m,\ \hat\tau_d]$, innovated by the position sensor alone. Every cycle it does two steps. Predict pushes the states forward using the mechanical model:
...
The measurement source is whatever the best position sensor is. Swapping sensors swaps the measurement source and the gain set, and nothing else.

What this observer owns is worth listing, because it is not everything. It owns $\hat\theta$, which the position loop closes on. It owns $\hat\tau_d$, so contact detection, the collision trip and the stall verdict all read it. And it owns its own $\hat\omega$ for every decision that is shaped around rest rather than around speed: the stall verdict pairs a pinned current command with $|\hat\omega|$ under a threshold, and the winding thermometer gates its update on a small $|\hat\omega|$ so that the back-EMF term it is neglecting really is negligible. That estimate is always there and it reads small at rest, and rest is exactly where the back-EMF has no drive window to measure in. What it is not is the velocity the velocity loop closes on.

### Which Velocity the Loop Closes On

![omega source switch](control/omega-switch.svg)

The servo has two velocity sources and they are good in opposite regimes, which is the whole reason there is a switch rather than a blend.

The back-EMF estimate is quiet, prompt and honest whenever the bridge is driving hard enough to have a window. It is blind at hold, where $\hat v_\text{diff}$ and $\hat R\cdot \hat i$ are the same size and a winding warmed 40 °C invents tens of rev/s of speed that is not there, and it is absent below the sampling floor. The pot is an honest position sensor at every speed, including zero, and a poor speedometer at any speed: its local gain swings by a factor of 5.8 across the travel, so differentiating it produces a position-dependent error that no window length averages away.

So the velocity loop takes the back-EMF boxcar while the boxcar is valid, and the pot observer's $\hat\omega$ otherwise. Two estimates, one selected, never mixed. The switch carries run-length hysteresis in both directions, and the two directions are deliberately asymmetric. Four consecutive valid boxcars, which is five clean halves or 2.5 ms at 20 kHz, are needed before the loop is handed to the back-EMF, so a duty hovering at the sampling floor cannot flip the source every half. Two consecutive voided results hand it back, which is the shortest run a real gap can produce, because a single sub-floor tick already voids two boxcars. Falling back fast is the right bias: a stale boxcar riding under the loop through a direction reversal is worse than pot noise. While the source is still the back-EMF, the last valid boxcar rides the loop through the gap.

Telemetry publishes the selected estimate as `omega_hat_cps` and the selection itself as `omega_hat_src`, next to the raw boxcar in `omega_bemf_cps`, so a capture always says which estimate the loop was holding when something went wrong.

Now the evidence, because this is the part of the design that measurement moved. An earlier version of this document had the back-EMF blended into the observer at a small weight, and the firmware carried an `l_bemf` gain to do it. Scored against a commutation-ripple reference on 80 settled rungs, the 1 ms back-EMF boxcar scatters by 0.62% where a pot derivative over a 20 ms window scatters by 5.40%, at a twentieth of the lag. Combining the two by inverse variance, which is arithmetic rather than taste, gives the pot one part in 17,804 of the velocity weight on rest covariances, or one part in 1,962 on the scatter the two estimators measurably have under drive. Either way the blend is not a blend, it is the back-EMF with a rounding error stapled to it. The noise densities cross at 1.92 Hz, and the observer's bandwidth is set an order of magnitude above that, in the region where the pot is orders of magnitude the noisier of the two, which is the arithmetic behind $\hat\omega$ railing by hundreds of counts per second while the shaft sits still.

Two further findings say the blend was the wrong shape and not merely the wrong weight. The pot's velocity error under drive is 14.5% rms of deterministic, position-locked gain error, repeating at a correlation of 0.997 between two halves of the captures that share no recording, against 2.2% of actual random noise. Inverse-variance weighting assumes zero-mean random error, so no weight and no filter averages that away. And through a direction reversal the two sources disagree because both are right: the motor is turning while the output is not, the teeth have not met yet, and a filter that treats them as measurements of the same state fights itself on every reversal. A switch has no opinion to fight with.
...
| $\hat\omega$ | back-EMF above the sampling floor, pot observer below | encoder + back-EMF | motor encoder |
...
Almost every block above leans on per-unit calibration data: the pot linearization table, the sin/cos ellipse fit, the IR curve, $K_e$, the $(R_0, T_0)$ anchor, the friction and cogging maps, the measured gear ratio, and the observer gains.
...
- The back-EMF observer is weakest at low speed and high current, which is unfortunately also where a servo spends a lot of its life. The winding thermometer recovers most of it, but the floor of this approach is set by how well $\hat R\cdot \hat i$ can be subtracted, and I don't expect miracles near stall. Below the sampling floor there is no estimate at all and the loop falls back to the pot. In the encoder-less configuration, that is the price of admission.
- The pot observer is a tradeoff forced by the dirt cheap MCU. A fancier filter would re-estimate the inertia as the load changes, but this chip has no room for matrix math, so the gains are baked offline from one identified $J$. What gets sacrificed is $\hat\tau_d$ during hard acceleration: when the real inertia is off from the identified one, the unexplained acceleration shows up as phantom external torque. Position stays honest, the correct step keeps re-anchoring it to real measurements, and a wrong $J$ costs nothing at steady speed because inertia only enters through acceleration. I think this is good enough, and if the bench says otherwise the fixes are incremental: calibrate $J$ with the real load attached, or mask contact detection while $|\alpha^{\ast}|$ is large. For what it's worth, ServoProject made the same call on the same class of chip: its Kalman filter also runs fixed gains computed offline, with no matrix math in the loop.
