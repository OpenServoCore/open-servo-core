...
- **Rule-eval flash tax on validated writes — data now, not code.** After the fixed-width read + word-mask + sorted-scan-break pass, the Write poll body sits at: 16 B unruled 10.8 µs, 1 B locked+hooked 7.75 µs, 4 B goal_position (two cross-register compares) 31.2 µs — ~11.5 µs per rule eval. The residual is measured to be *flash-resident rule data*, not code shape: RAM-placing the read leaf changed nothing, RAM-placing the whole eval bought only −5 µs for ~1.2 K of stack budget (rejected — band-2 wedge margin), and a case-split read was flat. `Rule`/`SectionMeta` statics live in `.rodata`, so every eval pays wait-stated flash *data* reads for its own fields. The real lever, if goal-rate writes ever need it: emit the rule tables as `static`s in `.data` (~300 B RAM for the current map) so eval's operands live in SRAM — a deliberate RAM-budget decision, deferred. Also measured and load-bearing: eval must stay `inline(never)` (inlining it into the rule-scan loop costs +8 µs on unruled 1 B writes), and runtime u64 shifts are `__ashldi3` libcalls on RV32E — edge masks are built in u32 with an explicit `== 32` branch.
