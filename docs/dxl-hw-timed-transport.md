...
- **Rule-eval cost is interpretation, not placement.** After the fixed-width read + word-mask + sorted-scan-break pass, the Write poll body sits at: 16 B unruled 10.8 µs, 1 B locked+hooked 7.75 µs, 4 B goal_position (two cross-register compares) 31.2 µs — ~10 µs per rule eval. Every placement lever was measured and came back flat: RAM-placing the read leaf (no change), the whole eval body (~8 µs/eval, −5 µs total for ~1.2 K stack — rejected), case-split reads (flat), and rule tables in `.data` (−0.9 µs for 384 B RAM — reverted, same standard as the band-2 RAM CRC table). Disassembly explains it: `Rule::eval` is a 468-instruction zero-call interpreter (kind match → RHS match → width/sign match → per-byte source-pick loops), and executing a branch-dense general interpreter step costs ~500 cycles on this core wherever code or data live. The real lever, if goal-rate writes ever need <15 µs: **rules-as-code** — have the derive emit each section's checks as straight-line monomorphic compares (`if goal < min { DataRange }`) instead of `Rule` data walked by an interpreter, which also deletes the `Rule`/`RuleKind`/`CmpOp` types and the eval machinery outright (data→code trade, ~2 K flash, zero RAM). Also measured and load-bearing: eval must stay `inline(never)` (inlining it into the rule-scan loop costs +8 µs on unruled 1 B writes), and runtime u64 shifts are `__ashldi3` libcalls on RV32E — edge masks are built in u32 with an explicit `== 32` branch.
