...
- A current-tick accessor (`packet_end_tick`) queried by the codec at event-handling time. Used for one-shot stamps — packet-end tick of an instruction the chip will reply to. The chain path for slots k > 0 uses neither (slot 0 uses the packet-end stamp via the single-target reply path).
...
- **Header pattern mismatch under EMI or HSI excursion.** Fails for that packet only. HSI drift sampling — the only consumer requiring byte-precise anchored ticks — is skipped: the drift accumulator's flag stays cleared throughout the body so unanchored samples cannot poison the calibration loop. Slot 0 single-target reply timing degrades to the ISR-time fallback. Chain slots k > 0 fire on predecessor skip-exhaust rather than on a stamped tick, so they are anchor-independent and unaffected. The next packet's `Event::Sync` re-attempts anchor from scratch.
