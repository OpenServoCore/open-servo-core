...
**Validate before commit.** The control table is one 1024-byte `repr(C)` register file (address *is* byte offset); a write is checked in precedence order — bounds, then the derive-generated writable bitmask, then the flat rule table — before any live byte changes. (The DXL-era build additionally ran a section-level `write_locked_by` torque guard after the rules; osc deleted it — only MGMT SAVE is torque-gated, `osc-native-protocol.md` §9.4.) A plain Write that passes commits straight into the register file (no staging); a RegWrite instead validates and parks its payload in the staging buffer until Action flushes it. A failed check touches no live registers, and the CRC gate upstream means only integrity-verified frames ever reach the dispatcher. Reads hand back a single contiguous slice of the table (reserved and skip bytes read back zero — the derive enforces zero padding), which the reply encoders fold directly.
