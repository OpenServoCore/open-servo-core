...
**Parser drives, edge parser serves.** The protocol parser consumes the RX DMA ring one byte at a time and emits typed events per protocol field. The edge parser — which turns falling-edge timestamps into per-byte start ticks — advances in lockstep with the byte parser: on every event the codec computes the byte delta the parser has consumed since the previous event and walks the edge parser forward by exactly that many byte-boundaries. The edge parser's anchor is therefore always the edge-ring tick of the parser's most-recently-consumed byte by construction — overshoot is structurally impossible. Per-byte ticks are consumed at event-handling time and discarded; nothing is stored in a derived ring.

**Per-packet anchoring.** The DXL header `FF FF FD 00` has a unique signature on the wire as bytes (DXL stuffing guarantees the sequence cannot occur in payload) and as falling edges (a five-edge pattern with distinctive inter-edge intervals followed by a guaranteed quiet window). The edge parser recovers its anchor from this signature at every parser `Event::Sync`; drift, glitches, and overruns cannot accumulate across packet boundaries.
...
These principles compose. The prior design's byte-time ring existed because the classifier produced byte ticks that the parser then read by index; once the parser drives, the edge parser hands ticks directly to each consumer at the moment they are useful, and the ring becomes a small set of scalars per consumer.
...
**Event granularity.** One event per protocol field: header match, instruction-header (id, instruction, addr, length), status-header (id, length, error), per-slot demarcations inside Sync Write / Bulk Write / Sync Read / Bulk Read parameters, payload region start and end markers, payload chunk markers carrying `(offset, size)` ranges into the RX DMA ring, CRC verdict (`Event::Crc(Good)` / `Event::Crc(Bad)` — both fire after the same 2 trailing CRC bytes so the codec's byte-delta math is structural rather than verdict-aware), and resync (mid-packet aborts: bad length, ring overflow). Payload data flows as ring-relative offsets — no borrow lifetime, no parser-side copy.
...
The parser drives. On the parser's `Event::Sync` (emitted once per packet after consuming the 4-byte `FF FF FD 00` signature), the edge parser walks the ET ring backward from its tail, looking for a five-entry window whose inter-entry deltas match the signature within tolerance, followed by the quiet gap before the next entry. A match yields the anchor: the start tick of byte 3 (the trailing `0x00`'s start bit, the 5th edge of the signature). The ET reader cursor advances to one past edge 5, so subsequent forward advances begin from byte 4.

The back-search latency budget is comfortable. By the time the parser emits `Event::Sync`, the 5 signature edges have been latched into ET for at least 4·byte_times. The search bound is at most a few entries past the most recent signature in the worst case; loose bounds are inexpensive.

### 4.3 Lockstep advance and tick exposure

The codec drives the edge parser in lockstep with the byte parser. `CodecRx` tracks `last_walked_pos` — the wire-byte position the edge-parser anchor currently represents. For every parser event other than `Event::Sync`, the codec computes `delta = next_status_pos − last_walked_pos`, where `next_status_pos` is the byte cursor at the event's emit point, and invokes `Rx::advance_walker(delta)`. The edge parser walks forward exactly `delta` byte-boundaries from the current anchor using the `[9·bit, 11·bit]` start-bit window inherited from the prior design, skipping intra-byte transitions (FD's bit-0→bit-1, AA's data-bit edges) along the way. At `Event::Sync` the back-search seeds the anchor at byte 3 and the codec resets `last_walked_pos = next_status_pos` directly — the 4 signature bytes don't need a forward walk because the back-search consumed them.

The universal byte-skip (§5.2) also drives the walker. Each batch of skipped bytes calls `advance_walker(batch_size)` before completing, so the edge parser stays in lockstep across foreign packets too. This preserves anchor coherence over the wire (so the next packet's back-search has a sensible recent edge-ring tail) and routes drift pairs through foreign Instruction bodies for HSI sampling per [[drift_sampling_instruction_only]].

Per-byte ticks reach consumers at two touch points:

- A current-tick accessor (`packet_end_tick`) queried by the codec at event-handling time. Used for one-shot stamps — packet-end tick of an instruction the chip will reply to. The Plain chain path for slots k > 0 uses neither (slot 0 uses the packet-end stamp via the single-target reply path).
- A FAST-chain status-start query (`status_start_tick`, task #142): at the per-byte RXNE wake a deferred FAST slot k > 0 resolves the start tick of the chain's single Status packet from the newest ET stamps. The packet's first byte is `0xFF` — exactly one falling edge — so on an on-time wake the newest frame-aged stamp IS the packet-start tick (with a one-frame-spaced pair disambiguating an in-flight second header byte). A late wake (two or more reply bytes in) falls back to the same tail-signature back-search the Crc path uses, run read-only over the newest arrived bytes, then walks back to byte 0 in whole frames — the reply burst is contiguous, so the walk-back is exact.
- A flag-gated `on_pair(prev, curr)` callback fired per HIT during walker advance, routed to the HSI drift accumulator. The flag is set at any Instruction-Header event (own or foreign — see above) and cleared at the matching packet boundary (`Event::Crc(_)`, `PollEvent::SkipComplete`, or `Event::Resync(_)`). Status packets never contribute; the source clock for drift sampling is always the host's HSE.

Per-packet timing state collapses to anchor, byte count, plus the drift accumulator's running scalars. Chain-participation state owns no ticks at all — a `predecessor_id` and a chain-pending flag, with the universal byte-skip's `skip_remaining` / `current_skip_id` riding alongside as foreign-packet handling state.

### 4.4 Freshness and failure modes

ET DMA captures every falling edge unconditionally. The codec refreshes the ET producer head on every ISR entry (HT/TC or IDLE) via NDTR readback and again at the entry of every `advance_walker` call, so the edge parser sees the freshest published edges every time it walks. With ET = 128 and ~3 edges/byte on random content, the ring holds well over a packet's worth of edges at all design bauds.

The lockstep contract gives the failure modes structural shape:

- **Late edges.** When advance_walker(n) is called and the next start-bit edge hasn't been DMA'd yet, the edge parser bails short (returns < n) and leaves the anchor intact. The codec's `last_walked_pos` only advances by the count returned; the next event's delta picks up the outstanding byte against newly-arrived edges. The walker catches up automatically.
- **Edge loss / HSI excursion mid-packet.** A start-bit edge that falls outside the `[9·bit, 11·bit]` window trips GAP. The walker drops the anchor; on its own this would leave `packet_end_tick` without a tick at the pending Crc. **Tail-signature backstop:** at every `Event::Crc(_)` the edge parser also runs an independent back-search against the just-received tail bytes' wire-edge pattern — same `[idle, start=0, d0..d7, stop=1]` falling-edge arithmetic the header signature uses, but the target pattern is computed from the per-packet tail (last 4 raw wire bytes the parser consumed) rather than the fixed `FF FF FD 00`. The hint comes from `EDGES_PER_BYTE` summed across the tail bytes (the first tail byte's start edge sits `total_edges − 1` slots back from the ring head, with `PADDING_EDGES_AHEAD` slack for in-flight skew); probing runs oldest-first so a low-entropy tail of trailing `0xFF` bytes can't be spoofed by a newer matching window. On match, `packet_end_tick` reads the tail anchor in preference to `last_byte_start`, so a within-packet GAP that dropped the walker no longer drops the timing signal. The walker still owns HSI integrator pair emission. **Both anchors miss → ISR-time TIM2 capture fallback** (`packet_end_tick_fallback`), which sits within RX DMA drain latency of the wire packet end — well inside the host's read window for any realistic RDT. **GAP is no longer a cross-packet defence; it is now a genuine anomaly signal** (the parser-driven walker can't overshoot, so a within-packet GAP means edges were lost or HSI drifted out of band).
- **Header pattern mismatch under EMI or HSI excursion.** Fails for that packet only. HSI drift sampling — the only consumer requiring byte-precise anchored ticks — is skipped: the drift accumulator's flag stays cleared throughout the body so unanchored samples cannot poison the calibration loop. Slot 0 single-target reply timing degrades to the ISR-time fallback. Plain chain slots k > 0 fire on predecessor skip-exhaust rather than on a stamped tick, so they are anchor-independent and unaffected. FAST chain slots k > 0 are anchor-DEPENDENT (§5.6): a missed status-start resolution retries on the next byte's wake (each new byte gives a fresh tail signature); if nothing resolvable ever arrives the slot simply never fires — the FAST collapse contract — and stale traffic past the staleness window drops the parked slot. The next packet's `Event::Sync` re-attempts anchor from scratch.
- **Boot / post-IDLE.** No timing state is wrong about anything yet — the codec's `last_walked_pos = 0` sentinel makes advance_walker a no-op until the first `Event::Sync` seeds the anchor.
...
**Total.** ET (256 B) + RX (64 B) + edge-parser scalars (~16 B) ≈ 336 B, ~4% of V006 SRAM. Doubling ET halves HT cadence and grows the lookback budget but adds 256 B for marginal benefit on typical traffic; halving ET drops 160 B but undercuts the lookback budget below typical packet edge counts. ET = 128 / RX = 64 is the design point at which all three pressures are simultaneously comfortable on V006.

## 5. Plain chain timing

### 5.1 Start rule

For chain replies at slots k > 0 (inside a Plain Sync Read or Plain Bulk Read), the chip starts the wire inline (`TxBus::start_now` — TX_EN force-active + TX DMA enable, no compare involved)

at the moment the parser-drain handler observes the byte-skip exhaust of the chip's immediate predecessor, matched by ID. There is no `predecessor_end + something` term, no `packet_end + RDT` term, no `max()`. The wait-for-predecessor guarantee is structural: a packet-end observation cannot occur until the predecessor's complete bytes have been consumed, which by definition means the predecessor has stopped driving the bus.
...
where `packet_end_tick` is the chain-instruction's packet-end stamped by the edge parser at the parser's `Event::Crc(Good)`. The slot 0 start is scheduled through the hardware TX kickoff ([dxl-hw-timed-transport.md §5.1](dxl-hw-timed-transport.md)), not in-handler, and behaves identically to a Ping or single-Read reply.

For slots k > 0, the chip does not wait for any subsequent event. The handler that observes the predecessor's packet-end starts the wire in the same invocation, regardless of which trigger drove it. No backdated timestamp survives in the chain path.
...
**Slot 0 as single-target.** When the chip's ID is the first slot demarcation, the chip behaves exactly as it would for a single-target Read reply. At the chain-instruction's `Event::Crc(Good)`, the edge parser stamps `packet_end_tick`; the chip then schedules the wire start at `packet_end_tick + RDT` through the hardware TX kickoff. Between the arm and the compare match, the chain instruction has finished, the bus has been idle for RDT, and the kickoff fires on its own. Slot 0 inherits the existing single-target reply infrastructure unchanged; no chain-pending state, no skip-exhaust check.
...
The integrator emits two corrections at every batch close: an integer-step HSITRIM write (committed at the next `on_rx_packet_end`, see [dxl-hw-timed-transport.md §10.7.2](dxl-hw-timed-transport.md)) and a Q8.8 sub-step residual that feeds `phase_adjust` into every fire deadline — `deadline = packet_end_tick + delay_ticks + phase_adjust` at both `send_status` and `send_slot` schedule sites. The residual handles the ±half-step quantization gap the integer HSITRIM register can't resolve.
...
- **Walker / parser ordering for the HSI flag.** **Resolved by the lockstep design.** The codec drives the edge parser before each event sink callback, so any per-byte tick for the bytes that produced the current event has already pushed before the sink runs. The Instruction Header sets the drift flag before the body's payload events are emitted, and `Event::Crc(_)` / `PollEvent::SkipComplete` / `Event::Resync(_)` clear the flag after the matching packet's last byte boundary has been walked.
- **Sustained-burst edge-parser cost.** The edge parser advances its anchor across every byte (own or skip-consumed) to keep the lockstep contract with the byte parser. Confirm under sustained 3 Mbaud bursts that edge parser + byte parser + event handling fits the ~25% CPU envelope of the prior design.
- **Parser-drain coverage across triggers.** The chain-start path relies on the parser advancing on all of USART1 IDLE, DMA1_CH5 HT, and DMA1_CH5 TC, with each handler's drain promptly observing the predecessor's skip-exhaust as bytes cross the ring tail. Confirm the runtime routes the parser into each handler with consistent ordering relative to the edge parser and the skip-exhaust check.
...
The design replaces the prior hardware-timed transport's derived byte-time ring and parser scratch with a streaming, event-driven byte parser walking the RX DMA ring directly and a parallel streaming edge parser walking the ET ring in lockstep. Per-byte ticks reach consumers via the edge parser's current-tick accessor and a flag-gated push to the drift accumulator, queried at event-handling time and discarded thereafter. Per-packet anchors are recovered from the DXL header's unique on-wire edge signature, guaranteed distinct by DXL stuffing, so timing drift never accumulates across packet boundaries. Foreign packets — in-chain predecessor packets, foreign single-target packets — are consumed by a universal byte-skip that keeps both parsers' view of the ring tail synchronized with the wire. Plain Sync / Bulk Read chain timing — left at fixed slot offsets in the prior design — splits cleanly between slot 0 and slots k > 0. Slot 0 inherits the single-target reply path unchanged (`packet_end + RDT`). Slots k > 0 reduce to a single piece of state, the immediate predecessor's ID, resolved at slot-demarcation parsing; the TX starts in-handler under whichever of USART1 IDLE, DMA1_CH5 HT, or DMA1_CH5 TC observes that ID's byte-skip exhaust. Nothing in the slot k > 0 chain path is scheduled, so nothing needs cleanup on stall — a silent predecessor manifests as a missing reply at the host's transaction layer. RDT applies at slot 0 (matching spec single-target semantics) and not at slots k > 0. The Fast successor chain-CRC pipeline shrinks to a checkpoint pickup off the predecessor's trailing CRC bytes, with no edge-parser involvement and O(own reply) CPU. Inter-slot gap on the wire sits at or below the observed Robotis baseline at typical deployment baud. Transport-owned RAM drops by ~352–384 B against the prior design; CPU during foreign-packet handling scales linearly with bytes the chip actually consumes rather than with bus traffic. The trigger model (TIM2 input capture, DMA1_CH5 HT/TC, USART1 IDLE) and priority allocation are unchanged; the Fast successor pipeline rides a single SysTick wake.
