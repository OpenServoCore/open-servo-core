...
- **Data-centric reply surface.** `DxlReply`'s methods (`send_status`, `send_status_read_chunked`, `send_slot_chunked`, `stage_*`) carry only *what* to reply, never *where on the wire* to position it — slot position, RDT, chain-CRC anchor, and packet-end timing all live inside the bus, derived from the request it just handed the dispatcher (driver-pattern §7.4). The `&mut R` reply handle borrows a disjoint field of bus state (`CodecTx`) while the framer's verdict borrows the RX ring (`CodecRx`); the `Codec::split_mut` sub-composite (driver-pattern §4.3) exposes the two halves at once.

**Validate before commit.** A Write's payload is decoded into the RegWrite/Action staging buffer and range/type/permission-checked against the control table before it commits; a failed check discards the staging without touching live registers, and the CRC gate upstream means only integrity-verified frames ever reach the dispatcher.
