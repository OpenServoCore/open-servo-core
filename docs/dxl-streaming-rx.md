...
- **Edge loss / HSI excursion mid-packet.** A start-bit edge that falls outside the `[9·bit, 11·bit]` window trips GAP. The walker drops the anchor and the codec's `packet_end_tick` returns None at the pending Crc. The fire scheduler falls back to the ISR-time TIM2 capture (`packet_end_tick_fallback`), which sits within RX DMA drain latency of the wire packet end — well inside the host's read window for any realistic RDT. **GAP is no longer a cross-packet defence; it is now a genuine anomaly signal** (the parser-driven walker can't overshoot, so a within-packet GAP means edges were lost or HSI drifted out of band).
